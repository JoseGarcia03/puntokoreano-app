...
export const MainLayout = ({ children }: Props) => {
    ...
    const items = [
        ...
        {
            key: 2,
            label: (<Link to={"/store"}>Tienda</Link>)
        },
        {
            key: 3,
    ...
    return (
        <Layout>
            <Header className={`flex justify-between items-center ${(isMobile || isTablet) && "px-5"}`} >
                <Flex align="center" gap={10} className="w-fit text-white flex-1" >
                    <img src={Logo} width={50} />
                    <h2 className="text-xl md:text-2xl w-36" >Punto Koreano</h2>
                </Flex>
                <Flex align="center" className="flex-1 lg:-ml-44 xl:-ml-62" justify={`${(isMobile || isTablet) ? "flex-end" : "space-between"}`}>
                    <Menu defaultSelectedKeys={['1']} className={`w-16 sm:w-48 lg:flex-1`} theme="dark" mode="horizontal" items={items} overflowedIndicator={<FontAwesomeIcon icon={faBars} />} />
