    ...
    return (
        <div className="w-full z-20 bg-white px-2">
            <div className="max-w-[1320px] mx-auto overflow-hidden lg:px-10">
                <div ref={ref} className="flex h-fit px-2 justify-center text-secondary_1 mx-5">
                    <Slider
                    ref={slider}
                    className="w-full"
                    dots={false}
                    ...
                    >
                    {
                        groupsData?.map((group: group, idx: number ) => {
                            return (
                                ...
                                className={`whitespace-nowrap text-center px-4 cursor-pointer ${ (hoveredCategory?.name === group.name) && "text-white bg-header" } transition-all duration-300`}
                                >
                                    { group.name }
                                </div>
                            )
