...
const CardProducts = ({ inline = false }: Props) => {
    ...
    return (
        <div className={`${ !inline && 'transition duration-300 hover:-translate-y-4 hover:shadow-2xl'}`}>
            <div data-aos={!inline && "fade-up"} className="transition duration-500 w-80 border rounded-lg relative cursor-pointer hover:border-[#E2060F] sm:w-64 md:w-72 xl:w-80">
                <div>
                    ...
                    <div className="h-80">
                        {
                            inline &&
                            <img
                            onClick={() => navigate('/store/product/1')}
                            src="https://risingtheme.com/html/demo-partsix/partsix/assets/img/product/main-product/product1.webp" alt="" className="rounded-lg w-full h-80 sm:w-64 sm:h-auto xl:w-80 object-scale-down"
                            />
                        }
                        { !inline && <Swiper
                        navigation
